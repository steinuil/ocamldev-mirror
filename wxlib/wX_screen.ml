          ...
          match pixmap with
            FromFile file ->
              let file = if Filename.is_relative file then
                  Filepath.find_in_path !pixmap_path file else file
              in
      ...
      Not_found -> try
          ...
          let cursor = 
            ...
            | BitmapCursor (bitmap,mask) ->
                let bitmap = Filepath.find_in_path !pixmap_path bitmap in
                let mask = Filepath.find_in_path !pixmap_path mask in
