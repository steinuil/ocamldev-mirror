...
(* $Id: iarray.ml,v 1.2 2001/07/26 16:38:29 lefessan Exp $ *)

(* Array operations *)

external length : int array -> int = "%array_length"
external get: int array -> int -> int = "%array_safe_get"
external set: int array -> int -> int -> unit = "%array_safe_set"
external unsafe_get: int array -> int -> int = "%array_unsafe_get"
external unsafe_set: int array -> int -> int -> unit = "%array_unsafe_set"
external make: int -> int -> int array = "make_ivect"
external create: int -> int -> int array = "make_ivect"

external array_sblit: int array -> int -> int array -> int -> int -> unit = "array_sblit" "noalloc"
let min_sblit_size = 50
...
let make_matrix sx sy init =
  let res = Parray.create sx [||] in
  for x = 0 to pred sx do
    Parray.unsafe_set res x (create sy init)
  done;
  res
...
let copy a =
  ...
  if l = 0 then [||] else begin
      ...
      if l < min_sblit_size then 
        for i = 1 to pred l do
          unsafe_set res i (unsafe_get a i)
        done
      else
        array_sblit a 0 res 0 l;
    res
  end
...
let blit a1 ofs1 a2 ofs2 len =
  ...
  else if len < min_sblit_size then
    if ofs1 < ofs2 then
(* Top-down copy *)
...
(* Bottom-up copy *)
  ...
  else array_sblit a1 ofs1 a2 ofs2 len
...
let mapi f a =
  let l = length a in
  if l = 0 then [||] else begin
    let r = Array.create l (f 0 (unsafe_get a 0)) in
    for i = 1 to l - 1 do
