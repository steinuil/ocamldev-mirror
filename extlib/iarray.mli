...
(* $Id: iarray.mli,v 1.2 2001/09/09 22:43:41 lefessan Exp $ *)
...
val init: int -> (int -> int) -> int array
        (* [Array.init n f] returns a fresh array of length [n],
           with element number [i] initialized to the result of [f i].
           In other terms, [Array.init n f] tabulates the results of [f]
           applied to the integers [0] to [n-1]. *)
val make_matrix: int -> int -> int -> int array array
val create_matrix: int -> int -> int -> int array array
        (* [Array.make_matrix dimx dimy e] returns a two-dimensional array
           ...
           *)
val append: int array -> int array -> int array
...
val blit: int array -> int ->
          int array -> int -> int -> unit
...
val map: (int -> 'b) -> int array -> 'b array
        (* [Array.map f a] applies function [f] to all the elements of [a],
           and builds an array with the results returned by [f]:
           [[| f a.(0); f a.(1); ...; f a.(Array.length a - 1) |]]. *)
val iteri: (int -> int -> unit) -> int array -> unit
val mapi: (int -> int -> 'b) -> int array -> 'b array
        (* Same as [Array.iter] and [Array.map] respectively, but the
           function is applied to the index of the element as first argument,
           and the element itself as second argument. *)
val fold_left: ('a -> int -> 'a) -> 'a -> int array -> 'a
        (* [Array.fold_left f x a] computes
           [f (... (f (f x a.(0)) a.(1)) ...) a.(n-1)],
           where [n] is the length of the array [a]. *)
val fold_right: (int -> 'a -> 'a) -> int array -> 'a -> 'a
        (* [Array.fold_right f a x] computes
           [f a.(0) (f a.(1) ( ... (f a.(n-1) x) ...))],
           where [n] is the length of the array [a]. *)

(** Sorting *)
val sort : (int -> int -> int) -> int array -> unit;;
...
val stable_sort : (int -> int -> int) -> int array -> unit;;
