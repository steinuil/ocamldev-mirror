...
let rec get_action action =
  ...
  | _ -> action
let _ = Printf.printf "OK"; print_newline ();;
let _ =
...
let value_to_image v = match v with
  ...
  | Options.Value s -> ScaleImage (Passwd.string_to_filename s)
  | Options.List [Options.Value "spixmap"; Options.Value s] ->
      ScaleImage (Passwd.string_to_filename s)
  | Options.List [Options.Value "tpixmap"; Options.Value s] ->
      TileImage (Passwd.string_to_filename s)
  | _ -> raise Not_found
...
let image_to_value v = 
  ...
  | ScaleImage s -> Options.SmallList 
      [Options.Value "spixmap"; Options.Value (Passwd.filename_to_string s)]
  | TileImage s -> Options.SmallList 
      [Options.Value "tpixmap"; Options.Value (Passwd.filename_to_string s)]
...
let find_pixmap filename =
  try
    Filepath.find_in_path !!graphics_path filename
  with e ->
      Printf.printf "No %s in graphics_path" filename;
      print_newline ();      
      List.iter (fun s -> Printf.printf "DIR %s" s; print_newline ();) !!graphics_path;
      raise e
