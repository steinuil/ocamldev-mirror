...
let send_ConfigInfo m w =
  ...
  List.iter (fun s ->
    ...
    (Printf.sprintf "PixmapPath %s" (Filepath.path_to_string !!icon_path))::
    (Printf.sprintf "IconPath %s" (Filepath.path_to_string !!icon_path)) ::
    !config_info
  );
...
(*             
  ...
  with
    e -> Printf.printf "Name %s" (Printexc.to_string e);
