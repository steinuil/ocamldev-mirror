...
let _ = 
  if !!smart_install then begin
      ...
      add_to_path_check asthemes_path (Filename.concat Passwd.home "GNUstep/Library/Afterstep/desktops/themes");
...
let binpath = 
  List.map Passwd.string_to_filename (Filepath.string_to_path
  (try
      Sys.getenv "PATH"
  ...
  | _ -> 
    ...
    root... *)
          if not (Sys.file_exists !setroot_cache) then begin
              try
          (* Enlightment version *)
                setroot_cache := Filepath.find_in_path binpath "Esetroot";
                setroot_cmd := "Esetroot"
              with Not_found ->
                  try
                (* WindowMaker version *)
                    setroot_cache := Filepath.find_in_path binpath "wmsetbg";
                    setroot_cmd := "wmsetbg -u -t"
                  with Not_found ->
                      try
                    (* Xloadimage version *)
                        setroot_cache := Filepath.find_in_path binpath "xsetbg";
                        setroot_cmd := "xsetbg"
                      with Not_found ->
                          try
                    (* xv version *)
                            setroot_cache := Filepath.find_in_path binpath "xv";
                            setroot_cmd := "xv -quit -root"
                          with Not_found -> ()
            end;
let cmd = Printf.sprintf "%s %s &" !setroot_cmd filename in
Printf.printf "Command bg: %s" cmd; print_newline ();
          let _ = command cmd
          in ()
        end
...
let set_background_theme name sw =
  let file = try
      Filepath.find_in_path !!asthemes_path 
...
let find_image image = 
  try
    Filepath.find_in_path (!!asthemes_path @ !!ethemes_path @ !!themes_path) image
  with
    Not_found -> 
