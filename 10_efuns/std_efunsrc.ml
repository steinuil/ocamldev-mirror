...
(*  define_action "eval" Complex.eval;   *)
  ...
  define_action "capitalize_word" 
    (fun frame ->
      let buf = frame.frm_buffer in
      let point = frame.frm_point in
      on_word buf point String.capitalize);
...
(* C-h map *)
  ...
  define_action "scroll_line_up" (fun l -> scroll_line l (-1));
  define_action "scroll_line_down" (fun l -> scroll_line l 1);
  define_action "scroll_line_up10" (fun l -> scroll_line l (-10));
  define_action "scroll_line_down10" (fun l -> scroll_line l 10);
  ()
...
let select_open_file frame =
  ...
  query#setWM_TRANSIENT_FOR top_window.top_appli#container;
  ...
  help_menu := [|
    ...
    "Changes", (fun frame ->
        let _ = Frame.load_file frame.frm_window (
            Efuns_version.efuns_lib ^"/Changes") in ());
