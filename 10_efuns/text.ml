...
module TextTree = WX_text.Make(LocalText)
open TextTree
  
type t = TextTree.tree TextTree.tree_desc  
...
let copy_res tree point len =
  let text = tree.tree_text in  
  let (pos,str) = low_copy tree point.point len in
  pos, str

let delete text point len = let _ = delete_res text point len in ()
  
let copy text point len = let _ = copy_res text point len in ()
...
let replace_matched tree repl =
  let text = tree.tree_text in    
  Str.replace_matched repl text.text_string
...
let tab_width = define_option ["tab_width"] "Size of tab"
  int_option 9
  
let init_tabreprs () =
  if !tabreprs = [||] then
    tabreprs := Array.init (!!tab_width) (fun i ->
        String.make (!!tab_width - i) ' '   
    )
