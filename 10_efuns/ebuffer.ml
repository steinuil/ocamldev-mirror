...
let create location name filename text local_map =
  ...
  buf.buf_charreprs.(9) <- String.make !!tab_width ' ';
  let hooks =  try
...
Version is " ^ Efuns_version.efuns_version ^"
with
Efuns installation directory : " ^ Efuns_version.efuns_lib ^ "
Ocaml installation directory : " ^ Cdk.ocamllib ^ "
...
See changes in "^ Efuns_version.efuns_lib ^"/Changes
"
        else ""
      in
...
let catch format buf f =
  ...
  with e ->
      let location = buf.buf_location in
      let name = "*Messages*" in
      let m = Printf.sprintf format (Printexc.to_string e) in
      try
        let buf = Hashtbl.find location.loc_buffers name in
        Text.insert_at_end buf.buf_text (m ^ "\n");
      with
