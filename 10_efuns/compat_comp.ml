...
module Type = struct 
    ...
    and iter_expression e list =
      ...
      | Texp_apply (e,l) ->
          List.fold_left (fun list e ->
              match e with None,_ -> list |
                (Some e),_ -> iter_expression e list
          ) (iter_expression e list) l
      | Texp_match (e,l,_) ->
          List.fold_left (fun list (pat,e) ->
              iter_pattern pat (iter_expression e list) Defined
      ...
      | Texp_variant (_, Some e) -> iter_expression e list    
      | Texp_variant _
      | Texp_instvar _
      | Texp_new _ 
    ...
    and iter_pattern pat list define =
      ...
      | Tpat_or (p1,p2,_) -> iter_pattern p1 (iter_pattern p2 list define) define
...
let format = Format.std_formatter
let printtyp_path p = Printtyp.path format p
let printtyp_type_expr t = Printtyp.type_expr format t

let compile_implementation = Compile.implementation
let errors_report_error =  Errors.report_error
let printtyp_cltype_declaration = Printtyp.cltype_declaration
let printtyp_class_declaration = Printtyp.class_declaration
let printtyp_modtype_declaration = Printtyp.modtype_declaration
let printtyp_modtype = Printtyp.modtype
let printtyp_exception_declaration = Printtyp.exception_declaration
let printtyp_type_declaration = Printtyp.type_declaration
let print_value =  Dyneval.print_value
let printtyp_type_scheme = Printtyp.type_scheme
let printtyp_ident = Printtyp.ident
let printinstr_instrlist =  Printinstr.instrlist
let printlambda_lambda =  Printlambda.lambda
