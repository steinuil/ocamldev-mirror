...
let create_window display parent ncols nlines =
  ...
  let mask = 
    [CWBackPixel (get_color display 1);
      CWEventMask [ExposureMask; KeyPressMask; KeyReleaseMask;
...
let last_click = ref None

let double_click_delay = 0.250
...
let xterm_handler window handler ev =
  ...
  match ev.ev_event with
    KeyPressEvent k ->
      ...
      if keysym = XK.xk_Mode_switch then begin
        display.dpy.dpy_mode_switch <- 1
      end else begin
  ...
  | ButtonPressEvent k ->
      ...
      handler (try
          match !last_click with 
            None -> raise Not_found
          | Some k2 ->
              if k.Xbutton.detail = k2.Xbutton.detail &&
                k.Xbutton.event = k2.Xbutton.event &&
                (real_time k.Xbutton.time) -. (real_time k2.Xbutton.time)
                < 1000. *. double_click_delay
              then
                (XTDoublePress (k.Xbutton.state,k.Xbutton.detail,
                ...
                raise Not_found
        with _ -> 
            (XTButtonPress (k.Xbutton.state,k.Xbutton.detail,
                (k.Xbutton.x_event - 2) / display.f_width,
                (k.Xbutton.y_event - 2) / display.f_height
              ));
      );
...
let set_selection xterm f =
  Selection.setSelection xterm#display xterm#window XA.xa_primary
    (fun target ->
      if target = XA.xa_string then 
        1, (f ()) else raise Not_found
  ) !Eloop.event_time
  
let set_property xterm prop value =
  let display = xterm.display in
  let atom = X.internAtom display.dpy prop false in
  X.changeProperty display.dpy display.root 
        PropModeReplace atom XA.xa_string 1 value;
