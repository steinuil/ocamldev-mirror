...
let _ =
  if !!graphics_path = [] then
    graphics_path =:= 
      ...
      (Filename.concat Passwd.home ".pixmaps");
      (Sys.getcwd ());
...
let logfile = define_option ["logfile"] 
  "<logfile> is the file where debugging information is stored of [logp] is
    true." filename_option (Filename.concat Passwd.home "gwml.log")
...
let xdefaults = try Sys.getenv "XUSERFILESEARCHPATH" with
    Not_found -> Filename.concat Passwd.home ".Xdefaults"
...
let _ =
  begin    
    try
      let gwml_res = 
        let path = try Filepath.string_to_path (Sys.getenv "XFILESEARCHPATH") with _ -> 
              [] in
...
let grabServer () =
  if not !Imager.server_grabbed then begin
      X.grabServer display;
      Imager.server_grabbed := true
    end else
    failwith "Already grabbed"
    
let ungrabServer () =
  if !Imager.server_grabbed then begin
      X.ungrabServer display;
      Imager.server_grabbed := false;
  ...
  try Hashtbl.find s.s_pixmaps name with Not_found ->
      ...
      let pixmap = 
        match pixmap with
          FromFile file ->
            (try
                let file = Filepath.find_in_path !!graphics_path file in
                (try
                    Xpm.createPixmapFromFile display scr.scr_root
                      scr.scr_default_colormap scr.scr_root_depth file
                  with _ when !!use_imlib && not !Imager.server_grabbed ->
(* Maybe we can use Imlib to render this image ... *)
              ...
              with e ->
                  Printf.printf "No %s found in graphics_path" file;
                  print_newline ();
                  raise e)
        | FromFunction (name,f) -> 
            let (pix,mask) = f w in
    ...
    Not_found ->
      ...
      let cursor = 
        ...
        | BitmapCursor (bitmap,mask) ->
            let bitmap = Filepath.find_in_path !!graphics_path bitmap in
            let mask = Filepath.find_in_path !!graphics_path mask in
