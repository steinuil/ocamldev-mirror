...
let twm_config _ =
  let rc = 
    try
      Twm.load (Filename.concat Passwd.home ".twmrc")
    with
...
let tvtwm_config _ =  
  let rc = 
    try
      Twm.load (Filename.concat Passwd.home ".tvtwmrc") 
    with
...
let fvwm_config _ =
  ...
  let rc = 
    try
      Fvwm.load proto (Filename.concat Passwd.home ".fvwmrc") 
    with
...
let fvwm2_config _ =
  ...
  let rc = try
      Fvwm.load proto (Filename.concat Passwd.home ".fvwm2rc") 
    with
...
let xterm_e cmd w =
  let _ = command (Printf.sprintf "(xterm -e %s)&" cmd) in ()

  
let fast_hosts_menu _ =
  List.map (fun (host, cmd) ->
      Printf.printf "MENU: %s --> %s" host cmd; print_newline ();
      host, [],Function (xterm_e cmd)
  ) !!preferred_hosts
  ...
  if !fvwm_menu = [] then 
    try
      ...
      let rc = Fvwm.load proto (
          Filename.concat Passwd.home !!Fvwm.fvwm_file) in
      Fvwm.use proto [] rc;
...
let help w =
  commandw (Printf.sprintf "efuns %s/FAQ.gwml &" gwml_lib) w
