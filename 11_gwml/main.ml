...
let _ =
  Printexc2.register_exn (fun exn ->
      try raise exn with
        XError e -> 
...
let gwmldir = Filename.concat Passwd.home
    (".gwml-" ^ gwml_version)
...
(*  Dyneval.init Format.std_formatter true; 
  ...
  if (not !batch_mode) || !gwml_talk then
    Printexc2.catchexn ".gwmlrc.ml:" (fun _ ->
        Printf.printf "Loading configuration file .gwmlrc.ml"; 
        print_newline ();
        
        let file = Filepath.find_in_path (Passwd.home :: !!load_path)
          ".gwmlrc.ml" in
...
(*  if !help then Options.help stdout; *)
  ...
  Gwml.grabServer ();
  Printexc2.catch "Decorate screen:" (fun _ ->
  ...
  let gwmlrc = Filename.concat Passwd.home ".gwmlrc" in
...
let _ = 
  Printexc2.catch "Main.main()" main ();
  Wob.exit_gwml ()
