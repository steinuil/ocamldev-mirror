...
let makefile_color buf = debug makefile_color buf
...
let install buf = debug install buf
...
let makefile_mode frame = 
  debug (fun mode -> Ebuffer.set_major_mode frame.frm_buffer mode) mode
...
let _ =
  ...
  define_action "makefile_mode.color_buffer" 
    (fun frame -> 
      debug
        makefile_color frame.frm_buffer
  );
