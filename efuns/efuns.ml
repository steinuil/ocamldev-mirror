...
(*
let efuns_path = [ 
    (Filename.concat Passwd.home ".efuns") ;
    efuns_lib;
    Cdk.ocamllib;
...
(*    "-I",Arg.String (fun s -> load_path =:= 
        (Filepath.string_to_path s) @ !!load_path), "<path>: Load Path"; *)
...
(** Max *)
    "-server", Arg.Set server_opt, ": start as a server";
(** Fin Max *)
...
let _ =
  Options.filename := 
  (try Filepath.find_in_path (Passwd.home :: !!load_path) ".efunsrc" with
      _ -> Filename.concat Passwd.home ".efunsrc"); 
...
(** Max *)
let server = define_option ["server"] "" bool_option false
(** Fin Max *)
