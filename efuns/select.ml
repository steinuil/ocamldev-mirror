...
let complete_filename frame good_file filename =
  if is_userdir filename then List.map (fun entry -> entry.Passwd.login) 
    (Passwd.get_entries ())
  else
  let filename = Passwd.string_to_filename filename in
      ...
      if not(!completion = !string) then
        ...
        let suffix = 
          ...
          if len>0 && s.[len - 1] <> '/' then
            try
              let filename = Passwd.string_to_filename s in
              let dirname = dirname frame filename in
...
(* can be improved, by displaying another part of the completion buffer *)
  ...
  (fun old_frame str -> 
      history := str :: !history;
      remove_completions frame;
      let str = Passwd.string_to_filename str in
      action str
  ) in
...
let select_filename frame request action =
  ...
  select_file frame request file_hist (Passwd.filename_to_string curdir) action
