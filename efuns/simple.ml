...
let delete_backward_word buf point =
  ...
  let _, str = Text.delete_res 
      text point (Text.distance text point old_point) 
    in
...
let delete_forward_word buf point =
  ...
  let _, str = Text.delete_res text point len in
  copy_string str
...
let copy_region frame =
  ...
  let (start,term) = 
    if mark > point then (point,mark) else (mark,point)
  in
  let _,region = copy_res text start (distance text start term) in
  copy_string region
...
let mouse_drag_region frame =
  ...
  WX_xterm.set_selection xterm (fun _ ->
      match !highlighted with
