...
(** Max *)
(* Save the file whose name is given. *)
let save_file window filename =
  let top_window = Window.top window in
  let location = top_window.top_location in
  try
    let buf = Hashtbl.find location.loc_files filename in
...
(* Return true if the given file is modified. *)
let file_modified window filename =
  let top_window = Window.top window in
  let location = top_window.top_location in
  try
    let buf = Hashtbl.find location.loc_files filename in
...
(* Return true if the given file is currently edited. *)
let file_edited window filename =
  let top_window = Window.top window in
  let location = top_window.top_location in
  try
    let buf = Hashtbl.find location.loc_files filename in
...
(* Kill the given file. *)
let kill_file window filename =
  let top_window = Window.top window in
  let location = top_window.top_location in
  try
    let buf = Hashtbl.find location.loc_files filename in
(* A VOIR si c'est comme ça qu'il faut faire
    let frame = create window None  buf in
    let new_buf = next_buffer location buf in
    let new_frame = Frame.create window None new_buf in
*)
    Ebuffer.kill location buf
  with
    Not_found -> 
      ()
(** Fin Max *)
