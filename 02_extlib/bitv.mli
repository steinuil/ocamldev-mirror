...
(* $Id: bitv.mli,v 1.3 2001/05/21 12:27:12 lefessan Exp $ *)

 (*d @strong{Module Bitv}.
    ...
    the same operations as the module [Array] . 
    It also provides bitwise operations . 
    In the following, [false] stands for the bit 0 
    and [true] for the bit 1. *)

type t

(*s @strong{Creation, access and assignment.} 
...
(*s @strong{Copies and concatenations.}
...
(*s @strong{Sub-vectors and filling.} 
    ...
    element number [o2]. It @emph{does not work} correctly if [v1] and [v2] are
...
(*s @strong{Conversions to and from strings.} 
