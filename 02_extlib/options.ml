...
let filename = ref (Filename.concat Sysenv.home
...
let value_to_path v =
  List.map Filename2.from_string
    (match v with Value s -> Filepath.string_to_path s
    | List l -> List.map (fun v -> match v with
              Value s -> Filename2.from_string s
                | _ -> raise Not_found) l
    | _ -> raise Not_found)
      
let path_to_value list = Value (Filepath.path_to_string 
      (List.map Filename2.to_string list))
  ...
  save_module "" oc (List.map (fun o -> 
        ...
        try o.option_class.to_value o.option_value with e -> 
            Printf.printf "Error while saving option \"%s\": %s" 
              (try List.hd o.option_name with _ -> "???") (Printexc.to_string e);
            print_newline ();
            Value "") 
    (List.rev !options));
...
let value_to_filename v =  Filename2.from_string (match v with
      Value s -> s | _ -> raise Not_found)
  
let filename_to_value v = Value (Filename2.to_string v)
