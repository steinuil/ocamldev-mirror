...
let ocamlbin="/usr/bin"
let ocamlver="3.09.1"
let ocamllib="/usr/lib/ocaml/3.09.1"
let srcdir="/home/lefessan/devel/ocamlconf"
open Genlex
...
let load _loc filename =
  values := (read_values filename) @ !values;
  ocamlver := List.assoc "ocamlver" !values;
  Printf.fprintf stderr "Version %s\n" !ocamlver;
...
EXTEND 
  ...
  Pcaml.str_item: FIRST
    ...
    | "DEFINE"; ident = UIDENT; operator = UIDENT; name = STRING ->
        ...
        in
        if v then begin
            Printf.fprintf stderr "DEFINE %s\n" ident;
